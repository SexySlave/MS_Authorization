...
//        System.out.println(userId);
    ...
    public String generateJWTPair(int useId) throws Exception {
        ...
        String jwtAccessToken = JWT.create().withIssuer("MS_AUTHORIZATION").withSubject("MS_AUTHORIZATION_user").withClaim("type", "accesstoken")
                ...
                .withIssuedAt(new Date()).withExpiresAt(new Date(System.currentTimeMillis() + 5000L)).withJWTId(UUID.randomUUID().toString()).withNotBefore(new Date(System.currentTimeMillis() - 1000L)).sign(algorithm);


        String jwtRefreshToken = JWT.create().withIssuer("MS_AUTHORIZATION").withSubject("MS_AUTHORIZATION_user").withClaim("type", "refreshtoken").withClaim("version", "15647839049гоалвд234").withIssuedAt(new Date()).withExpiresAt(new Date(System.currentTimeMillis() + 1000L * 60 * 60 * 24 * 60)) // 60 days
    ...
    public String generateRefreshJWT(int userId, Boolean updateVersion) {
        ...
        String jwtRefreshToken = JWT.create().withIssuer("MS_AUTHORIZATION").withSubject("MS_AUTHORIZATION_user").withClaim("type", "refreshtoken")
                //.withClaim("redreshtokenUUID", )
                .withClaim("version", getRefreshTokenVersion(userId)).withIssuedAt(new Date()).withExpiresAt(new Date(System.currentTimeMillis() + 1000L * 60 * 60 * 24 * 60)) // 60 days
                .withJWTId(UUID.randomUUID().toString()).withNotBefore(new Date(System.currentTimeMillis() - 1000L)).sign(algorithm);
        return jwtRefreshToken;
    ...
    public String generateRefreshJWT(Boolean updateVersion) {
        ...
        String jwtRefreshToken = JWT.create().withIssuer("MS_AUTHORIZATION").withSubject("MS_AUTHORIZATION_user").withClaim("type", "refreshtoken").withClaim("version", user.getJWTversion()).withClaim("refreshtokenuuid", user.getRefreshtokenUUID()).withIssuedAt(new Date()).withExpiresAt(new Date(System.currentTimeMillis() + 1000L * 60 * 60 * 24 * 60)) // 60 days
                .withJWTId(UUID.randomUUID().toString()).withNotBefore(new Date(System.currentTimeMillis() - 1000L)).sign(algorithm);
        return jwtRefreshToken;
    }
    ...
    public String generateRefereshJWTFromJWT(String refreshJWT) {  // using only when obtaining refreshtoken
        ...
        String jwtRefreshToken = JWT.create().withIssuer("MS_AUTHORIZATION").withSubject("MS_AUTHORIZATION_user").withClaim("type", "refreshtokenuuid").withClaim("version", user.getJWTversion()).withClaim("refreshtokenuuid", user.getRefreshtokenUUID()).withIssuedAt(new Date()).withExpiresAt(new Date(System.currentTimeMillis() + 1000L * 60 * 60 * 24 * 60)) // 60 days
                .withJWTId(UUID.randomUUID().toString()).withNotBefore(new Date(System.currentTimeMillis() - 1000L)).sign(algorithm);
        return jwtRefreshToken;
    ...
    private int getRefreshTokenVersion(int userId) {
        ...
        Session session = sessionFactory.openSession();
        int JWTversion = session.createQuery("SELECT JWTversion from UsersDefault where id =:i", int.class).setParameter("i", userId).getSingleResultOrNull();
    ...
    private void updateRefreshTokenVersion(int userId) {
        ...
        try {
            session.createMutationQuery("update UsersDefault set JWTversion = :v where id = :i").setParameter("i", userId).setParameter("v", (session.createQuery("SELECT JWTversion from UsersDefault where id =:i", int.class).setParameter("i", userId).getSingleResultOrNull()) + 1).executeUpdate();
            transaction.commit();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void updateRefreshTokenVersionByUUID(int refreshTokenUUID) {
        ...
        Transaction transaction = session.beginTransaction();
        try {
            session.createMutationQuery("update UsersDefault set JWTversion = :v where refreshtokenUUID = :i").setParameter("i", refreshTokenUUID).setParameter("v", (session.createQuery("SELECT JWTversion from UsersDefault where refreshtokenUUID =:i", int.class).setParameter("i", refreshTokenUUID).getSingleResultOrNull()) + 1).executeUpdate();
            transaction.commit();
        } catch (Exception e) {
            e.printStackTrace();
            transaction.rollback();
